...
max_iter = 5  # nombre d’itérations VQPCA

# === 4. Initialisation des clusters (aléatoire) ===
np.random.seed(0)
idx = np.random.randint(0, k, size=X0_U.shape[0])
...
for iteration in range(max_iter):
    ...
    cluster_means = {}
    cluster_scalers = {}
    X0_rec = np.zeros_like(X0_U)

    for i in range(k):
        cluster_points = X0_U[idx == i]
...
err = reconstruction_error(X_U, X_rec)
