...
np.random.seed(0)
idx = np.random.randint(0, k, size=X0_VORT.shape[0])
...
for iteration in range(max_iter):
    ...
    cluster_means = {}
    cluster_scalers = {}
    X0_rec = np.zeros_like(X0_VORT)

    for i in range(k):
        cluster_points = X0_VORT[idx == i]
    ...
    for i in range(X0_VORT.shape[0]):
        errors = []
        for c in range(k):
...
u_orig = VORTALL[:, t].reshape((nx, ny))
...
err = reconstruction_error(X_VORT, X_rec)
