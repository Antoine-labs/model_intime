...
VALL = data['VALL']  # shape (np, nt)
nx = data["ny"][0][0]
ny = data["nx"][0][0]
nt = VALL.shape[1]

# Transpose : chaque ligne = snapshot temporel
X_V = VALL.T  # shape (nt, np)
...
cluster_labels_V = kmeans.fit_predict(X0_V)

# === 4. Local PCA dans chaque cluster ===
q = 6  # nombre de composantes principales à garder
X0_rec_local_V = np.zeros_like(X0_V)

for k in range(optimal_k):
    mask = (cluster_labels_V == k)
    X_k = X0_V[mask]
    ...
    X0_rec_local_V[mask] = X_k_rec

# === 5. Retour à l’échelle d’origine
X_rec_local_V = scaler.inverse_transform(X0_rec_local_V)  # shape (nt, np)

# === 6. Visualisation d’un snapshot reconstruit ===
x = np.linspace(-1, 8, nx)
y = np.linspace(-2, 2, ny)
X, Y = np.meshgrid(x, y, indexing='ij')

t = 50  # timestep
v_orig = VALL[:, t].reshape((nx, ny))
v_rec = X_rec_local_V[t, :].reshape((nx, ny))

fig, axes = plt.subplots(1, 2, figsize=(12, 5))
levels = np.linspace(-1.0, 1.0, 100)

cf1 = axes[0].contourf(X, Y, v_orig, levels=levels, cmap='viridis')
axes[0].set_title(f'Original V (t={t})')
axes[0].set_xlabel('x / D')
axes[0].set_ylabel('y / D')

cf2 = axes[1].contourf(X, Y, v_rec, levels=levels, cmap='viridis')
axes[1].set_title(f'Reconstructed V (Local PCA, q={q}, t={t})')
...
err_V = reconstruction_error(X_V, X_rec_local_V)
print(f"Erreur relative de reconstruction (Local PCA sur V): {err_V:.5f} %")