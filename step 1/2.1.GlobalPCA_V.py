...
###########################################################################
#%% Streamwise component of the velocity, uy


X_V = VALL.T  # shape (nt_U, np)
...
plt.scatter(Z_V[:, 2], Z_V[:, 3], c=np.arange(nt_V), cmap='viridis', s=50)
plt.xlabel('PC1')
plt.ylabel('PC2')
...
X0_rec_V = Z_V[:, :q] @ pca.components_[:q, :]
X_rec_V = scaler.inverse_transform(X0_rec_V)

# === 8. Plot original vs reconstructed at timestep t ===
t = 50  # Time snapshot to display
u_orig_V = UALL[:, t].reshape((nx, ny))       # original
u_rec_V  = X_rec_V[t, :].reshape((nx, ny))      # reconstructed

fig, axes = plt.subplots(1, 2, figsize=(12, 5))
levels = np.linspace(-1.5, 1.5, 100)

cf1 = axes[0].contourf(X, Y, u_orig_V, levels=levels, cmap='viridis')
axes[0].set_title(f'Original $u_x$ at t={t}')
axes[0].set_xlabel('x / D')
axes[0].set_ylabel('y / D')

cf2 = axes[1].contourf(X, Y, u_rec_V, levels=levels, cmap='viridis')
...
err = reconstruction_error(X_V, X_rec_V)
